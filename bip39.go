...
var (
	AllSupportedLanguages = []language.Tag{
		language.English,
	...
	// wordList is the set of words to use. Key is language.
	wordLists map[language.Tag][]string = make(map[language.Tag][]string)
	// wordMap is a reverse lookup map for wordList. Key is language.
	wordMaps map[language.Tag]map[string]int
)
...
func init() {
	ClearLanauges()
	AddSupportedLanguage(language.English)
}

// ClearLanauges clears all supported languages.
func ClearLanauges() {
	wordLists = make(map[language.Tag][]string)
	wordMaps = make(map[language.Tag]map[string]int)
}

func AddSupportedLanguage(tag language.Tag) {
	switch tag {
	case language.English:
		AddWordList(tag, wordlists.English)
	case language.SimplifiedChinese:
		AddWordList(tag, wordlists.ChineseSimplified)
	case language.TraditionalChinese:
		AddWordList(tag, wordlists.ChineseTraditional)
	case language.Czech:
		AddWordList(tag, wordlists.Czech)
	case language.French:
		AddWordList(tag, wordlists.French)
	case language.Italian:
		AddWordList(tag, wordlists.Italian)
	case language.Japanese:
		AddWordList(tag, wordlists.Japanese)
	case language.Korean:
		AddWordList(tag, wordlists.Korean)
	case language.Spanish:
		AddWordList(tag, wordlists.Spanish)
	default:
		panic("unsupported language")
	}
}

func AddAllSupportedLanguages() {
	for _, lang := range AllSupportedLanguages {
		AddSupportedLanguage(lang)
	}
}

// AddWordList addes the list of words to use for mnemonics. Currently the list
// that is set is used package-wide.
func AddWordList(langauge language.Tag, list []string) {
	wordLists[langauge] = list
	wordMaps[langauge] = make(map[string]int)
	for i, v := range list {
		wordMaps[langauge][v] = i
	}
}

// GetWordList gets the list of words to use for mnemonics.
func GetWordList(lang language.Tag) []string {
	return wordLists[lang]
}

// GetWordIndex gets word index in wordMap.
func GetWordIndex(word string, lang language.Tag) (int, bool) {
	idx, ok := wordMaps[lang][word]
	return idx, ok
}
...
func findAllIndexes(words []string) ([]int, bool) {
	for _, wordMap := range wordMaps {
...
func EntropyFromMnemonic(mnemonic string) ([]byte, error) {
	...
	indexes, found := findAllIndexes(mnemonicSlice)
	if !found {
		return nil, ErrInvalidMnemonic
	}
	for _, v := range indexes {
		binary.BigEndian.PutUint16(wordBytes[:], uint16(v))
...
func NewMnemonic(entropy []byte, lang language.Tag) (string, error) {
	...
	for i := sentenceLength - 1; i >= 0; i-- {
		...
		words[i] = wordLists[lang][binary.BigEndian.Uint16(wordBytes)]
	}
